...
<head>
    ...
    <script>
        function Welcome() {
            <?php
            if (!isset($_SESSION["firstLogin"]) && !isset($_POST["除法"])) {
            ...
            document.addForm.除法.focus();
        }

        function check() {
            var 除法 = document.addForm.除法.value;
            if (除法.length == 0) {
                alert('注意:\n\n請提供作答內容!');
                document.addForm.除法.focus();
                return false;
                ...
                <div class="card text-center border-warning">
                    <h2 class="card-header bg-info text-dark">
                        <form action="" method="post">
                            請問<?php echo "$x3 / $y3 ="; ?>
                            <input type="text" name="除法">
                            <input class="btn btn-success" type="submit" value="確認作答">
                            <input type="hidden" name="x3" value="<?php echo $x3; ?>">
                            <input type="hidden" name="y3" value="<?php echo $y3; ?>">
